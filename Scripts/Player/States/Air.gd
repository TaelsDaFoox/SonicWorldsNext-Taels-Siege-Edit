...
func _process(_delta):
	...
	if parent.action3_pressed() and parent.character==Global.CHARACTERS.RUSHSONIC:
		...
		parent.set_state(parent.STATES.AIRDASH)
	if parent.playerControl != 0 and parent.any_action_pressed() and parent.character == Global.CHARACTERS.MIGHTY:
		...
		if parent.horizontalSensor.is_colliding() and !parent.ground:
			...
			parent.set_state(parent.STATES.WALLCLING)#,parent.currentHitbox.GLIDE
			# play grab sound
			parent.sfx[26].play()
		...
		# Shield actions
			...
			# Normal actions
				...
				match (parent.character):
					...
					# Mighty air abilities
					Global.CHARACTERS.MIGHTY:
						print(parent.currentState)
						if parent.currentState == 2:
							# set ability used to true to prevent multiple uses
							parent.abilityUsed = true
							if parent.inputs[parent.INPUTS.YINPUT] > 0:
								parent.movement.y=600.0
								parent.movement.x=0.0
								parent.sfx[3].play()
							else:
								parent.sfx[16].play()
								parent.shieldSprite.play("Insta")
